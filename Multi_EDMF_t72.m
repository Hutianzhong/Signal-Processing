...
str = '/Users/hutianzhong/Desktop/基于自适应加权多尺度组合形态滤波的轴承故障特征提取研究/数据/t72.txt';
c = load(str);
x = c.';
N = 2048; %采样点数
l = 0:N-1; 
...
WMDIF = weight(y1,Fc - Fo ,high) * (Fc - Fo);
...
WMOCCO = weight(y1,(Foc + Fco) /  2,high) * (Foc + Fco) /  2;
...
hil = abs(hil); %包络信号 
figure(23); plot(t,hil); title('WMEDMF 信号'); xlabel('时间/s'); ylabel('幅值A/(m/s^2)');
figure(22); NFFT = 2^nextpow2(N); Y = fft(hil,NFFT)/N; f = fs/2*linspace(0,1,NFFT/2+1); plot(f(4:fxy),2*abs(Y(4:fxy))); xlabel('频率f/Hz'); ylabel('幅值A/(m/s^2)');
